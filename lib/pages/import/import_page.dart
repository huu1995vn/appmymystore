...
import 'package:mymystore/pages/import/widgets/item_import.widget.dart';
import 'package:mymystore/pages/product/widgets/item_product.widget.dart';
import 'package:scroll_to_index/scroll_to_index.dart';

class ImportPage extends StatefulWidget {
  static String route() => '/import';
...
class _ImportPageState extends State<ImportPage> {
  ...
  loadData([nPaging = 1]) async {
    ...
    //   if (res.status > 0) {
    //     List<ImportModel> list = CommonMethods.convertToList<ImportModel>(
    //         res.data, (val) => ImportModel.fromJson(val));
    //     setState(() {
            ...
            child: MMCustomScrollView(
              ...
              slivers: <Widget>[
                MMSliverList(listData, (BuildContext context, int index) {
                  ...
                  return ItemImportWidget(
                    item,
                    onTap: () =>
                        {CommonNavigates.toImportPage(context, item: item)},
                  );
                })
              ],
            )));
