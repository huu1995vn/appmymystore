...
import 'package:raoxe/pages/setting/components/selected_theme_indicator.dart';
import 'package:raoxe/pages/setting/components/switcher_container.dart';
import 'package:raoxe/pages/setting/components/theme_option.dart';
  ...
  Widget build(BuildContext context) {
    double themeOptionIndicatorWidth =
        (MediaQuery.of(context).size.width - (20 * 4)) /
            ThemeService.appThemeOptions.length;
      ...
      content: SizedBox(
        ...
        child: Consumer<ThemeProvider>(
          builder: (c, themeProvider, _) {
            int selectedThemeIndex = ThemeService.appThemeOptions.indexWhere(
                ...
                Positioned.fill(
                  child: Row(
                    children: List.generate(
                      ...
                      (i) => ThemeOption(
                        key: Key(
                          '__${ThemeService.appThemeOptions[i].mode.name}_theme_option__',
                        ),
                        appTheme: ThemeService.appThemeOptions[i],
                        height: themeSwitcherOptionsHeight,
                        isSelected: selectedThemeIndex == i,
                        onTap: () => themeProvider.setSelectedThemeMode(
                          ThemeService.appThemeOptions[i].mode,
                        ),
                      ),
                    ),
                  ),
